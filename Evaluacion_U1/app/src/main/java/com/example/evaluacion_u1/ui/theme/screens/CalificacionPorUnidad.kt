...
@Composable
fun CalificacionUnidad2(navController: NavController, viewModel: DataViewModel) {
    val CalificacionUnida = viewModel.calificacionunidad
...
fun getCalificacionUnidad2(
    ...
    val service = RetrofitClient(context).retrofitService6
    val bodyAcademico = cargarRequestBody()
    service.getCalificacionPorUnidad(bodyAcademico).enqueue(object : Callback<Envelope6> {
        override fun onResponse(call: Call<Envelope6>, response: Response<Envelope6>) {
            if (response.isSuccessful) {
                val envelope = response.body()
                val alumnoResultJson: String? =
                    envelope?.body6?.getCalifUnidadesByAlumnoResponse?.getCalifUnidadesByAlumnoResult

                // Deserializa la cadena JSON a AlumnoAcademicoResult
                val json = Json { ignoreUnknownKeys = true }
                val CalificacionUnidadItem: List<CalificacionPUnidad>? =
                ...
                viewModel.calificacionunidad = CalificacionUnidadItem
        ...
        override fun onFailure(call: Call<Envelope6>, t: Throwable) {
            t.printStackTrace()
